---
title: "资源"
description: "向 LLM 暴露来自服务器的数据和内容"
---

资源是模型上下文协议（MCP）中的一个核心基元，它允许服务器暴露可被客户端读取的数据和内容，并用作 LLM 交互的上下文。

<Note>
  资源被设计为**应用程序控制**的，这意味着客户端应用程序可以决定如何以及何时使用它们。
  不同的 MCP 客户端可能会以不同方式处理资源。例如：
  - Claude Desktop 目前要求用户在使用资源之前明确选择资源
  - 其他客户端可能会基于启发式算法自动选择资源
  - 某些实现甚至可能允许 AI 模型自行决定使用哪些资源

  在实现资源支持时，服务器作者应该准备好处理这些任何交互模式。为了自动向模型公开数据，服务器作者应该使用**模型控制**的基元，比如 [工具](./tools)。
</Note>
...
对于动态资源，服务器可以公开 [URI 模板](https://datatracker.ietf.org/doc/html/rfc6570)，客户端可以使用这些模板构造有效的资源 URI：
...
服务器可以通过 `notifications/resources/list_changed` 通知来告知客户端其可用资源列表的变化。
