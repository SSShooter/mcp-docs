---
title: 调试
description: Model Context Protocol (MCP) 集成调试的完整指南
---
...
在使用带有 Claude Desktop 的 MCP 服务器时：
...
MCP 服务器只自动继承部分环境变量，如 `USER`、`HOME` 和 `PATH`。
...
对于所有[传输](/docs/concepts/transports)，你也可以通过发送日志消息通知来向客户端提供日志：
